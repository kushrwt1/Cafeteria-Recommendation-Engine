...
export class EmployeeProfileRepository {
    public async createProfile(profile: EmployeeProfile): Promise<void> {
        await db.execute(
            ...
            [profile.user_id, profile.dietary_preference, profile.spice_level, profile.cuisine_preference, profile.sweet_tooth, profile.profile_update_date]
        );
    ...
    public async updateProfile(profile: EmployeeProfile): Promise<void> {
        ...
        await db.execute(
            ...
            [profile.dietary_preference, profile.spice_level, profile.cuisine_preference, profile.sweet_tooth, profile.profile_update_date, profile.user_id]
        );
        console.log('User Profile Updated Successfully!');
    }

    public async getEmployeeProfileByUserId(user_id: number): Promise<EmployeeProfile | null> {
        const [rows] = await db.execute('SELECT * FROM Employee_Profile WHERE user_id = ?', [user_id]);
