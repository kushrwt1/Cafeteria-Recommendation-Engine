...
export class MealTypeRepository {
  ...
  public async getMealTypeById(id: number): Promise<MealType | null> {
    const [rows] = await db.execute("SELECT * FROM Meal_Types WHERE id = ?", [
      id,
    ]);
  ...
  public async addMealType(mealType: MealType): Promise<void> {
    await db.execute("INSERT INTO Meal_Types (meal_name) VALUES (?)", [
      mealType.meal_name,
    ]);
  }

  public async updateMealType(mealType: MealType): Promise<void> {
    await db.execute("UPDATE Meal_Types SET meal_name = ? WHERE id = ?", [
      mealType.meal_name,
      mealType.id,
    ]);
  }
