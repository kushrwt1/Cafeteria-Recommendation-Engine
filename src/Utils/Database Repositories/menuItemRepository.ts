...
export class MenuItemRepository {
    ...
    public async addMenuItem(menuItem: MenuItem, socket: net.Socket): Promise<void> {
        await db.execute('INSERT INTO Menu_Items (name, availability, price, meal_type_id, dietary_type, spice_level, cuisine_type, is_sweet) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [menuItem.name, menuItem.availability, menuItem.price, menuItem.meal_type_id, menuItem.dietary_type, menuItem.spice_level, menuItem.cuisine_type, menuItem.is_sweet]);
        socket.write("Menu Item Added Successfully");
        console.log("Menu Item added successfully");
    }

    public async updateMenuItem(menuItem: MenuItem): Promise<void> {
        await db.execute('UPDATE Menu_Items SET name = ?, availability = ?, price = ?, meal_type_id = ?, dietary_type = ?, spice_level = ?, cuisine_type = ?, is_sweet = ? WHERE menu_item_id = ?', [menuItem.name, menuItem.availability, menuItem.price, menuItem.meal_type_id, menuItem.dietary_type, menuItem.spice_level, menuItem.cuisine_type, menuItem.is_sweet, menuItem.menu_item_id]);
