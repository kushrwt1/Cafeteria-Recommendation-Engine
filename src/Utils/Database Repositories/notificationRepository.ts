...
export class NotificationRepository {
  ...
  public async getNotificationsByUserId(
    ...
    const [rows] = await db.execute(
      "SELECT * FROM notifications WHERE user_id = ? AND seen = false ORDER BY date DESC",
      [userId]
    );
    return rows as Notification[];
  }
  ...
  public async addNotification(notification: Notification): Promise<void> {
    await db.execute(
      "INSERT INTO notifications (user_id, message, date, seen) VALUES (?, ?, ?, ?)",
      [
  ...
  public async addNotificationForAllUsers(
    message: string,
    date: string
  ): Promise<void> {
    const query = `
  ...
  public async updateNotificationSeenStatus(
    notificationId: number
  ): Promise<void> {
    await db.execute("UPDATE notifications SET seen = true WHERE id = ?", [
      notificationId,
    ]);
  }
  ...
  public async updateNotification(notification: Notification): Promise<void> {
    await db.execute(
      "UPDATE Notifications SET message = ?, user_id = ? WHERE id = ?",
      [notification.message, notification.user_id, notification.id]
    );
  }
}
